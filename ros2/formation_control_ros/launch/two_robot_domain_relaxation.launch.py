"""Two-BlueROV sensing-domain relaxation paper experiment.

Purpose
-------
Isolate the adaptive sensing-domain mechanism in a simple two-robot setting.

The follower first tracks a relative position close to, but strictly inside,
the conservative maximum-distance boundary d_max^c = 3.0 m.  A short leader
motion is then applied approximately along the parent-minus-follower direction.
Because the follower cannot react instantaneously, the measured inter-robot
distance is driven transiently into the relaxable interval

    3.0 m < d < 3.6 m,

while remaining inside the physical sensing domain.

The desired formation itself always remains strictly inside the conservative
distance domain.  Thus any nonzero range-relaxation state is caused by the
transient, rather than by an infeasible desired reference.
"""
...
# pair_range_preload has norm ~= 2.90 m:
#   sqrt(1.05^2 + 2.70^2) ~= 2.897 m < d_max_conservative = 3.0 m.
#
# This leaves a small conservative margin which can be crossed dynamically by
# the leader pulse while remaining far from the physical limit d_max = 3.6 m.
...
def generate_launch_description() -> LaunchDescription:
    ...
    common = {
        ...
        "dry_run": dry_run,
        # Dynamic-layer tuning: the damped values used in the paper simulation.
        ...
        # Keep the pool barrier enabled, but make it non-adaptive by default so
        # that the plotted relaxation event is specifically a sensing-domain
        # event rather than a workspace-domain event.
    ...
    return LaunchDescription(
        [
            ...
            # Damped tuning used for the current paper simulations.
