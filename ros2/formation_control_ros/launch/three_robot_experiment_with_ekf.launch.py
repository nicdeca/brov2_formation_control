"""Three-robot hardware experiment using the revised MoCap estimator.
...
def _setup(context):
    ...
    follower_left = _perform(context, "follower_left")
    follower_right = _perform(context, "follower_right")
    ...
    ekf_arguments.update(
        {
            "robots": f"{leader},{follower_left},{follower_right}",
    ...
    experiment = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            "follower_left_robot_configuration": _perform(
                context, "follower_left_robot_configuration"
            ),
            "follower_right_robot_configuration": _perform(
                context, "follower_right_robot_configuration"
            ),
...
def generate_launch_description() -> LaunchDescription:
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "follower_left", default_value="itrl_rov_2"
            ),
            DeclareLaunchArgument(
            ...
            DeclareLaunchArgument(
                "virtual_linear_gain", default_value="0.55"
            ),
            DeclareLaunchArgument(
                "virtual_angular_gain", default_value="0.80"
            ),
            DeclareLaunchArgument(
            ...
            DeclareLaunchArgument(
                "publish_rate_hz", default_value="80.0"
            ),
            DeclareLaunchArgument(
            ...
            DeclareLaunchArgument(
                "gyro_timeout_sec", default_value="0.20"
            ),
            DeclareLaunchArgument(
                "gyro_time_constant_sec", default_value="0.03"
            ),
            DeclareLaunchArgument(
                "gyro_std", default_value="0.03"
            ),
            DeclareLaunchArgument(
                "max_gyro_abs_rad_s", default_value="5.0"
            ),
            DeclareLaunchArgument(
                "position_std", default_value="0.01"
            ),
            DeclareLaunchArgument(
                "linear_accel_std", default_value="0.7"
            ),
            DeclareLaunchArgument(
                "initial_velocity_std", default_value="0.5"
            ),
            DeclareLaunchArgument(
                "orientation_std", default_value="0.015"
            ),
            DeclareLaunchArgument(
                "orientation_measurement_gain",
                default_value="1.0",
            ...
            DeclareLaunchArgument(
                ...
                default_value="2.0",
                description=(
                    "Estimator health-check interval. Healthy operation "
                    "is silent; only problems are logged."
                ),
            ),
